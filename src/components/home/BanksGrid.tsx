  ...
  return (
    <section>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {banks.map(({ slug, label, sub, icon: Icon, active }) => {
          const content = (
            <>
          ...
          return active ? (
            <Link key={slug} to={`/banks/${slug}`}>
              <Card className="relative flex flex-col items-center text-center gap-2 hover:border-primary-200 border border-transparent">
                {content}
              </Card>
            </Link>
          ) : (
            <Card
              key={slug}
              className="relative flex flex-col items-center text-center gap-2 opacity-90"
            >
              {content}
            </Card>
          )
